...
function adduser (username, email, password, conf) {
  ...
  return fetch.defaults(conf.opts)(target, {
    method: 'PUT',
...
function login (username, password, conf) {
  ...
  const headers = authHeaders(conf.auth)
  headers['Content-Type'] = 'application/json'
  const logObj = {}
  Object.keys(userobj).forEach(k => {
    logObj[k] = k === 'password' ? 'XXXXX' : userobj[k]
...
function get (conf) {
  ...
  const headers = authHeaders(conf.auth)
  return fetch.defaults(conf.opts)(target, {headers: headers}).then(res => {
    if (res.status < 200 || res.status >= 300) {
      const err = new Error(res.statusText)
      err.code = res.status
      err.headers = res.headers.raw()
...
function listTokens (conf) {
  ...
  if (!conf.opts) conf.opts = {}
  const headers = authHeaders(conf.auth)
  const listFetch = fetch.defaults(conf.opts)

  return untilLastPage(`-/npm/v1/tokens`)

  function untilLastPage (href, objects) {
    return listFetch(url.resolve(conf.registry, href), {
      method: 'GET',
      headers: headers
    }).then(res => {
      if (res.status < 200 || res.status >= 300) {
