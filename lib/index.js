  ...
  return loginWeb(opener, conf).catch(er => {
    if (er instanceof WebLoginNotSupported) {
      ...
      return prompter(conf.creds)
        .then(data => loginCouch(data.name, data.password, conf))
    } else {
      throw er
    }
  })
...
function adduser (opener, prompter, conf) {
  ...
  return adduserWeb(opener, conf).catch(er => {
    if (er instanceof WebLoginNotSupported) {
      ...
      return prompter()
        .then(data => adduserCouch(data.name, data.email, data.password, conf))
    } else {
      throw er
    }
  })
...
function loginWeb (opener, conf) {
  ...
  return webAuth(opener, conf, null)
}
...
function webAuth (opener, conf, body) {
  ...
  return fetchJSON({
    ...
    function onDone (result) {
      ...
      if (res.status === 200) {
        if (!content.token) {
          throw new WebLoginInvalidResponse('GET', target, res, content)
        } else {
          return content
        }
